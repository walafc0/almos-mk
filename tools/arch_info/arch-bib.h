...
struct arch_bib_header_s
{
	...
	uint16_t   iopic_cluster;	//FG : ==-1 if no iopic and equal to the cluster which will deal with the wti if not
...
{
	uint8_t cid;//FG : if type changed, change also the type of arrays and id in boot_info_s
	uint8_t arch_cid;
	uint8_t  cpu_nr;
	uint8_t  dev_nr;
	uint16_t offset;
