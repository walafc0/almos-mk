...
gomp_free_thread (void *arg __attribute__((unused)))
{
...
#ifndef HAVE_TLS
  pthread_setspecific(gomp_tls_key, arg);
#endif
  ...
  if (pool)
	...
	{
	  int i;
	  for (i = 1; i < (int)pool->threads_used; i++)
	    {
		 ...
		 struct gomp_team *team)
{
  ...
  /* Launch new threads.  */
  int n_master;
  int error = pthread_attr_getcpuid_np(&n_master);
  if (error != 0) {
    fprintf(stderr, "ERROR: %s: failed to get cpu id\n", __FUNCTION__);
    n_master = -1;
  }
...
void initialize_team (void)
{
  ...
  if((err=pthread_key_create (&gomp_tls_key, NULL)))
    gomp_fatal ("could not create gomp_tls_key");


  if((err=pthread_setspecific (gomp_tls_key, &initial_thread_tls_data)))
    gomp_fatal ("could not seting initial thread tls data");
#endif
